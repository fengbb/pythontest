...
for size in logsize1:
        size = size.split('\t')
        if len(size) <= 1:     #chu li zi fu chuang chang du bu yi yang qing kuang
                continue
#       print size[0]    #look if size[0] is our dream data 
        if (200000 < int(size[0]) and int(size[0])< 1000000):
#               print size[1]  # test if success
                os.remove(size[1])
	if (int(size[0]) < 1000):  #shan chu .txt jie wei de wen jian
		os.remove(size[1])
